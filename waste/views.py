...
def staff_required(view_func):
    """Managers and Supervisors can log and view waste."""
    @login_required(login_url='my-login')
    def wrapper(request, *args, **kwargs):
        if not _is_admin_user(request.user):
            messages.error(request, 'You do not have permission to access waste management.')
...
def waste_list(request):
    qs = WasteLog.objects.select_related('logged_by').prefetch_related('items')
...
def waste_create(request):
    ...
    return render(request, 'waste/waste_create.html', {
        'items': items,
        'all_items': all_items,
        'reason_choices': WasteLog.REASON_CHOICES,
...
def waste_summary(request):
    ...
    return render(request, 'waste/waste_summary.html', {
        'restaurant': restaurant,
        'currency_symbol': restaurant.currency_symbol,
        'date_from': date_from,
        'date_to': date_to,
